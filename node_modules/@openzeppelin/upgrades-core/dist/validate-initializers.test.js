...
function testAccepts(name, kind) {
    testOverride(name, kind, {}, undefined);
}
function testRejects(name, kind, expectedErrors) {
    testOverride(name, kind, {}, expectedErrors);
}
function testOverride(name, kind, opts, expectedErrors) {
    ...
    test(testName, t => {
        ...
        if (expectValid) {
            ...
            const error = t.throws(assertUpgSafe);
            t.true(error.errors.length === expectedErrors.count, `Expected ${expectedErrors.count} errors, got ${error.errors.length}:\n${error.message}`);
            t.true(expectedErrors?.contains.every(c => error.message.includes(c)), error.message);
        }
    });
}
...
//# sourceMappingURL=validate-initializers.test.js.map