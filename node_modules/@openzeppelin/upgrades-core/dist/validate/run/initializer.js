 ...
 */
function getParentNameToInitializersMap(linearizedParentContracts) {
    const map = new Map();
 ...
 */
function getRecursiveFunctionIds(referencedFn, deref, visited) {
    const result = [];
 ...
 */
function getPossibleInitializers(contractDef, isParentContract) {
    const fns = [...(0, utils_1.findAll)('FunctionDefinition', contractDef)];
    return fns.filter(fnDef => (fnDef.modifiers.some(modifier => ['initializer', 'onlyInitializing'].includes(modifier.modifierName.name)) ||
